...
internal sealed class User : AuthUserInfo, IUser, IScopedDependency
{
    ...
    /// <returns></returns>
    public async Task<AuthUserInfo> GetAuthUserInfo(AppEnvironmentEnum deviceType, string tenantNo, string employeeNo)
    {
        // 获取缓存Key
        var cacheKey = CacheConst.GetCacheKey(CacheConst.AuthUserInfo, tenantNo, deviceType.ToString(), employeeNo);
    ...
    public async Task Login(AuthUserInfo authUserInfo)
    {
        ...
        try
        {
            ...
            var accessToken = JwtBearerUtil.GenerateToken(new Dictionary<string, object>
            {
                ...
                {nameof(EmployeeName), authUserInfo.EmployeeName},
    ...
    public async Task Logout()
    {
        ...
        if (!string.IsNullOrWhiteSpace(accessToken))
        {
            ...
            // 从 AccessToken 中读取 DeviceType,DeviceId,TenantNo,EmployeeNo
            ...
            var tenantNo = accessTokenIdentity.Claims.FirstOrDefault(f => f.Type == nameof(TenantNo))!.Value;
            var employeeNo = accessTokenIdentity.Claims.FirstOrDefault(f => f.Type == nameof(EmployeeNo))!.Value;

            // 尝试获取缓存
            var cacheKey = CacheConst.GetCacheKey(CacheConst.AuthUserInfo, tenantNo, deviceType, employeeNo);
