...
public class LoginService : IDynamicApplication
{
    ...
    private async Task<LoginOutput> HandleWeChatLogin(ApplicationModel applicationModel,
    ...
    public async Task<LoginOutput> WeChatLogin(WeChatLoginInput input)
    {
        ...
        var client = WechatApiClientBuilder
            .Create(new WechatApiClientOptions
                { AppId = applicationModel.OpenId, AppSecret = applicationModel.OpenSecret })
            .Build();
        var response =
            await client.ExecuteSnsJsCode2SessionAsync(new SnsJsCode2SessionRequest { JsCode = input.WeChatCode });
    ...
    public async Task<WeChatClientLoginOutput> WeChatClientLogin(WeChatClientLoginInput input)
    {
        ...
        else if (applicationModel.AppType == AppEnvironmentEnum.WeChatServiceAccount)
        {
            ...
            var tokenResponse =
                await client.ExecuteSnsOAuth2AccessTokenAsync(new SnsOAuth2AccessTokenRequest
                    { Code = input.WeChatCode });
