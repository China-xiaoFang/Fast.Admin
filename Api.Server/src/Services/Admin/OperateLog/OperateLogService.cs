...
public class OperateLogService : IDynamicApplication
{
    ...
    public OperateLogService(IUser user, ISqlSugarRepository<OperateLogModel> repository)
    {
        _user = user;
    ...
    public async Task<PagedResult<OperateLogModel>> QueryOperateLogPaged(QueryOperateLogPagedInput input)
    {
        ...
        if (_user.IsAdmin)
        {
            queryable = queryable.WhereIF(input.EmployeeId != null, wh => wh.EmployeeId == input.EmployeeId);
        }
        else
        {
