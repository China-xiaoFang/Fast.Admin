...
public class RoleService : IDynamicApplication
{
    ...
    public async Task<PagedResult<QueryRolePagedOutput>> QueryRolePaged(QueryRolePagedInput input)
    {
        return await _repository.Entities
            .WhereIF(!string.IsNullOrEmpty(input.RoleName), wh => wh.RoleName.Contains(input.RoleName))
            .WhereIF(!string.IsNullOrEmpty(input.RoleCode), wh => wh.RoleCode.Contains(input.RoleCode))
            .WhereIF(input.Status != null, wh => wh.Status == input.Status)
            .OrderByDescending(ob => ob.CreatedTime)
            .ToPagedListAsync(input, sl => new QueryRolePagedOutput
            {
                RoleId = sl.RoleId,
    ...
    public async Task DeleteRole(RoleIdInput input)
    {
        ...
        {
            throw new UserFriendlyException("角色存在员工关联，无法删除！");
        }
    ...
    /// <returns></returns>
    [HttpPost]
    [ApiInfo("分配菜单权限", HttpRequestActionEnum.Edit)]
    [Permission(PermissionConst.Role.AuthorizeMenus)]
    public async Task AuthorizeMenus(AuthorizeMenusInput input)
    {
        var roleModel = await _repository.SingleOrDefaultAsync(input.RoleId);
        if (roleModel == null)
        {
        ...
        // 验证菜单是否都存在
        var menuIds = input.MenuIds ?? new List<long>();
        if (menuIds.Any())
        {
            var existMenuIds = await _repository.Queryable<MenuModel>()
                .Where(wh => menuIds.Contains(wh.MenuId))
                .Select(sl => sl.MenuId)
                .ToListAsync();

            if (existMenuIds.Count != menuIds.Count)
            {
                throw new UserFriendlyException("部分菜单不存在！");
            }
        }
        ...
        await _repository.Ado.UseTranAsync(async () =>
        {
            ...
            if (menuIds.Any())
            {
                var roleMenuList = menuIds.Select(menuId => new RoleMenuModel
                {
                    RoleId = input.RoleId,
                    MenuId = menuId
                }).ToList();

                await _repository.Insertable(roleMenuList).ExecuteCommandAsync();
            }
        }, ex => throw ex);
    }
    ...
    /// <returns></returns>
    [HttpPost]
    [ApiInfo("分配按钮权限", HttpRequestActionEnum.Edit)]
    [Permission(PermissionConst.Role.AuthorizeButtons)]
    public async Task AuthorizeButtons(AuthorizeButtonsInput input)
    {
        var roleModel = await _repository.SingleOrDefaultAsync(input.RoleId);
        if (roleModel == null)
        {
        ...
        // 验证按钮是否都存在
        var buttonIds = input.ButtonIds ?? new List<long>();
        if (buttonIds.Any())
        {
            ...
            {
                throw new UserFriendlyException("部分按钮不存在！");
            }
        }
        ...
        await _repository.Ado.UseTranAsync(async () =>
        {
            ...
            if (buttonIds.Any())
            {
                var roleButtonList = buttonIds.Select(buttonId => new RoleButtonModel
                {
                    RoleId = input.RoleId,
                    ButtonId = buttonId
                }).ToList();

                await _repository.Insertable(roleButtonList).ExecuteCommandAsync();
            }
        }, ex => throw ex);
    }
    ...
    /// <returns></returns>
    [HttpGet]
    [ApiInfo("查询角色菜单", HttpRequestActionEnum.Query)]
    [Permission(PermissionConst.Role.QueryMenus)]
    public async Task<List<long>> QueryRoleMenus([Required(ErrorMessage = "角色Id不能为空")] long? roleId)
    {
        return await _repository.Queryable<RoleMenuModel>()
            .Where(wh => wh.RoleId == roleId)
            .Select(sl => sl.MenuId)
            .ToListAsync();
    }
    ...
    /// <returns></returns>
    [HttpGet]
    [ApiInfo("查询角色按钮", HttpRequestActionEnum.Query)]
    [Permission(PermissionConst.Role.QueryButtons)]
    public async Task<List<long>> QueryRoleButtons([Required(ErrorMessage = "角色Id不能为空")] long? roleId)
    {
        return await _repository.Queryable<RoleButtonModel>()
            .Where(wh => wh.RoleId == roleId)
            .Select(sl => sl.ButtonId)
            .ToListAsync();
    }
