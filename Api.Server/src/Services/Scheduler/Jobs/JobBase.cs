...
internal abstract class JobBase<T> : IJob where T : SchedulerJobLogInfo, new()
{
    ...
    protected JobBase(IServiceProvider serviceProvider, IMailService mailService,
    ...
    /// <returns></returns>
    protected abstract Task JobExecute(IServiceProvider serviceProvider, ISqlSugarClient db,
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        // 邮件消息
        MailMessage =
            context.JobDetail.JobDataMap.GetNullableEnum<MailMessageEnum>(nameof(SchedulerJobInfo.MailMessage))
        ...
        if (_logInfo.TenantId != null)
        {
            ...
            var isAllTenant = context.JobDetail.JobDataMap.GetNullableBoolean(nameof(SchedulerJobInfo.IsAllTenant))
                              ?? false;
            if (isAllTenant)
            {
                // 尝试获取本地调度作业的实现类
