	...
	>
		<FaForm ref="faFormRef" :model="state.formData" :rules="state.formRules" :disabled="state.formDisabled" cols="2" labelWidth="150">
			...
			<FaFormItem prop="visible" label="显示">
				<RadioGroup button name="YesOrNotEnum" v-model="state.formData.visible" />
			</FaFormItem>
...
const state = reactive({
	...
	formRules: withDefineType<FormRules>({
		...
		webTab: [{ required: true, message: "请选择Web端页面是否在导航栏显示", trigger: "change" }],
		webKeepAlive: [{ required: true, message: "请选择Web端页面是否缓存", trigger: "change" }],
		hasMobile: [{ required: true, message: "请选择是否移动端", trigger: "change" }],
